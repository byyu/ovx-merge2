...
public class OVXPacketIn extends OFPacketIn implements Virtualizable {
    ...
    public void virtualize(final PhysicalSwitch sw) {
        ...
        if (this.port.isEdge()) {
            ...
            //kllaf
            this.learnAddresses(match, map, vSwitch);
...
//                    eth.getDestinationMAC());
            ...
            if (match.getDataLayerType() == Ethernet.TYPE_ARP) {
            	...
            	try {
                    ...
                    if (this.tenantId == null) {
                        this.tenantId = (match.getTransportSource() << 16) | match.getTransportDestination();
                    }
    ...
    //kllaf
    private void learnAddresses(final OFMatch match, final Mappable map, OVXSwitch sw) {
        if (match.getDataLayerType() == Ethernet.TYPE_IPV4
                || match.getDataLayerType() == Ethernet.TYPE_ARP) {
        	int a;
            if (!match.getWildcardObj().isWildcarded(Flag.NW_SRC)) {
                a = IPMapper.getPhysicalIp(this.tenantId, match.getNetworkSource(), sw);
                System.out.println("kllaf Srcphy : " + a + "vir : " + match.getNetworkSource());
            }
            /*
            if (!match.getWildcardObj().isWildcarded(Flag.NW_DST)) {
                a = IPMapper.getPhysicalIp(this.tenantId,match.getNetworkDestination(), sw);
                System.out.println("kllaf Dstphy : " + a + "vir : " + match.getNetworkDestination());
            }
            */
        }
    }
