    ...
    private void prepAndSendSouth(OVXPort inPort, boolean pflag) {
        ...
        try {
            if (inPort.isEdge()) {
            	...
            	this.prependEncapAction(sw.getTenantId());
            	//this.prependRewriteActions();
            } else {
                //kllaf
            	IPMapper.rewriteMatch(sw.getTenantId(), this.match, sw);
